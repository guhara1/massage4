...
export function generateStaticParams() {
  return RESERVATION_SECTIONS.map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const s = findReservationSection(slug);
  if (!s) return {};
  return {
    ...
    alternates: { canonical: `/reservation/${slug}/` },
  };
}
...
export default async function ReservationDetailPage({ params }: Props) {
  ...
  const others = RESERVATION_SECTIONS.filter((x) => x.slug !== s.slug);
  const crumbs = [
    { name: "예약 안내", url: "/reservation/" },
    { name: s.name, url: `/reservation/${s.slug}/` },
  ];
  ...
  return (
    <article className="space-y-14">
      ...
      <nav className="flex items-center gap-2 text-xs text-white/60">
        <Link href="/reservation/" className="hover:text-brand-600">예약 안내</Link>
      ...
      <section className="space-y-5">
        <SectionHeader title="다른 예약 안내도 살펴보세요" />
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {others.map((o) => (
            <Link
              key={o.slug}
              href={`/reservation/${o.slug}/`}

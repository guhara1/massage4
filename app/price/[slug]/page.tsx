...
export function generateStaticParams() {
  return PRICE_SECTIONS.map((s) => ({ slug: s.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const s = findPriceSection(slug);
  if (!s) return {};
  return {
    ...
    alternates: { canonical: `/price/${slug}/` },
  };
}
...
export default async function PriceDetailPage({ params }: Props) {
  ...
  return (
    <article className="space-y-14">
      ...
      <section className="space-y-5">
        <SectionHeader title="다른 가격 안내도 살펴보세요" />
      ...
      <section className="rounded-2xl border border-brand-200 bg-brand-100 p-6 text-center sm:p-8">
        <h2 className="text-xl font-bold text-brand-800 sm:text-2xl">
          가격 확인이 끝났다면 바로 예약해 보세요
        </h2>
        <p className="mt-2 text-sm text-white/80">
          상담 시 출장비 포함 정확한 금액을 바로 안내해 드립니다.
        </p>
